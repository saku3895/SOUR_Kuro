...
# ✂️ 長いモデル名から「yolo26-nano」の部分を自動で引っ張ってくる
...
with dai.Pipeline(device) as pipeline:
    ...
    def displayFrame(name, frame, device_epoch_time=None):
        ...
        for d_idx, detection in enumerate(detections):
            ...
            # 骨格線の描画（YOLO26n / COCO 17 keypoints 完全版）
            YOLO26_EDGES = [
                (0, 1), (0, 2),   # 鼻 -> 左目, 右目
                (1, 3), (2, 4),   # 目 -> 耳
                (3, 5), (4, 6),   # 耳 -> 肩
            ...
            if is_recording and current_csv_file is not None:
                ...
                row_data = [
                    ...
                    str(log_frame_id), 
                    f"{instant_fps:.1f}", 
                    f"{latency_ms:.1f}",  # 🔥 ここに補正済みの綺麗なミリ秒値が入ります
                    f"{person_depth_m:.2f}"
                ]
    ...
    while pipeline.isRunning():
        ...
        if inRgb is not None:
            ...
            # 🔥 getTimestampDevice() を呼び出して、カメラの露光時刻をラズパイ時間（秒）に完全同期
