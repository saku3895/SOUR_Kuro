...
def execute_periodic_task(self, lock, data_dict):
    ...
    # データ取得
    gyro_data = self.mpu6886.get_gyro_data()    #ジャイロ
    accel_data = self.mpu6886.get_accel_data()  #加速度

    #numpy.arrayへ変換
    na_gyro_data = numpy.array(gyro_data)
    na_accel_data = numpy.array(accel_data)

    temp = self.mpu6886.get_temp_data()         #内部温度

    #madgwickフィルタで姿勢推定
    self.ahrs.update_no_magnetometer(na_gyro_data, na_accel_data, 0.01) #第３引数は更新間隔(0.01 = 1 / 100(Hz)).モジュールの実行間隔と合わせる．
    euler_angles = self.ahrs.quaternion.to_euler()  #オイラー角に
...
def execute_periodic_task(self, lock, data_dict):
    ...
    # 共有メモリに保存
    with lock:
        data_dict['pitch'] = euler_angles[0]
        data_dict['roll'] = euler_angles[1]
        data_dict['yaw'] = euler_angles[2]
        data_dict['temp'] = temp
```
...
class PMMyCustomModule(PeriodicModule):
    def __init__(self, robot_properties, interval_ms=1000):
        """
        初期化処理
