...
│   │   ├── __init__.py
│   │   ├── lobot_serial_communication.py    # Lobotサーボ通信
│   │   ├── unitv2_serial_communication.py   # UnitV2通信
│   │   ├── mpu6886.py                       # MPU6886通信
│   │   └── recieve_motion_lang.py           # モーション言語受信
│   │
│   ├── control/                   # 制御ロジック
│   │   └── motion_lang2motor.py   # モーション言語→モータ変換
│   │
│   └── test/                      # テスト・デモ
│       ├── __init__.py
...
│       ├── motion_lang_test_shiro.py  # モーション言語テスト
│       └── motion.py              # モーション定義
│
└── wiki/                          # ドキュメント (本ディレクトリ)
    ├── 00-目次.md
...
| [async_module.py](../src/core/async_module.py) | 非同期モジュールの基底 | `AsyncModule` |
...
| ファイル | 説明 | プロトコル |
|---------|------|-----------|
| [lobot_serial_communication.py](../src/communication/lobot_serial_communication.py) | Lobotサーボ通信 | シリアル (独自プロトコル) |
...
| [recieve_motion_lang.py](../src/communication/recieve_motion_lang.py) | モーション言語受信 | UDP |
...
| ファイル | 説明 |
|---------|------|
| [motion_lang2motor.py](../src/control/motion_lang2motor.py) | モーション言語をサーボコマンドに変換 |
...
| [motion_lang_test_shiro.py](../src/test/motion_lang_test_shiro.py) | モーション言語テスト | Shiro |
...
| ディレクトリ | ファイル数 | 主な役割 |
|------------|-----------|---------|
| `src/core/` | 4 | フレームワークの基盤 |
| `src/modules/` | 4 | 再利用可能なモジュール |
| `src/communication/` | 4 | ハードウェア通信 |
| `src/control/` | 1 | 制御アルゴリズム |
| `src/test/` | 5 | 実装例とテスト |
