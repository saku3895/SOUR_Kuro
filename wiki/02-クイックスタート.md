...
ロボット動作に必要な各種モジュールを登録します。
- Hiwonder Serial Bus Servo Controllerを使う場合の例
...
ロボットの制御部分を担当するカスタム制御モジュールを作成し，登録します．
新たに定期実行モジュールを作成する場合はPeriodicModuleを継承したサブクラスを実装します．
...
- [../src/test/pm_demo_betty.py](../src/test/pm_demo_betty.py) - コミュニケーションロボットBettyのデモ
- [../src/test/pm_demo_shiro.py](../src/test/pm_demo_shiro.py) - ４脚ロボットShiroのデモ
